    ...
    unsigned int recvsz)
{
    ...
    cu->cu_sendsz = sendsz;
    cu->cu_recvsz = recvsz;
    call_msg.rm_xid = (uint32_t)(((uint32_t)rt_thread_self()) ^ ((uint32_t)rt_tick_get()) ^ (xid_count++));
    ...
    if (*sockp < 0)
    {
        *sockp = sal_socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
        if (*sockp < 0)
        {
    ...
    rt_mutex_init(&cu->mutex, "nfs_mutex", 0);
    return (cl);
    ...
    struct timeval utimeout)
{
    ...
    int ret;
    int retry = 3;
    rt_mutex_take(&cu->mutex, RT_WAITING_FOREVER);
call_again:
    xdrs = &(cu->cu_outxdrs);
    ...
    if ((!xdr_u_long(xdrs, (long *) &proc)) ||
            (!AUTH_MARSHALL(cl->cl_auth, xdrs)) || (!(*xargs) (xdrs, argsp)))
    {
        cu->cu_error.re_status = RPC_CANTENCODEARGS;
        rt_mutex_release(&cu->mutex);
        return RPC_CANTENCODEARGS;
    }
    outlen = (int) XDR_GETPOS(xdrs);

send_again:
    if (sal_sendto(cu->cu_sock, cu->cu_outbuf, outlen, 0,
    ...
    do
    {
        ...
        inlen = sal_recvfrom(cu->cu_sock, cu->cu_inbuf,
                         (int) cu->cu_recvsz, 0,
...
static bool_t clntudp_control(CLIENT *cl, int request, char *info)
{
    ...
    case CLSET_TIMEOUT:
        ...
        sal_setsockopt(cu->cu_sock, SOL_SOCKET, SO_RCVTIMEO,
